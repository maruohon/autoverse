...
public class MessageSyncTileEntity implements IMessage
{
    ...
    public void toBytes(ByteBuf buf)
    {
        ...
        {
            AutoverseByteBufUtils.writeItemStackToBuffer(buf, this.stacks[i]);
        }
    }
    ...
    public void fromBytes(ByteBuf buf)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < len; i++)
            {
                this.stacks[i] = AutoverseByteBufUtils.readItemStackFromBuffer(buf);
            }
        }
        catch (IOException e)
        {
            Autoverse.logger.warn("MessageSyncTileEntity: Exception while reading data from buffer", e);
        }
    }
    ...
    public static class Handler implements IMessageHandler<MessageSyncTileEntity, IMessage>
    {
        ...
        public IMessage onMessage(final MessageSyncTileEntity message, MessageContext ctx)
        {
            ...
            {
                Autoverse.logger.error("Wrong side in MessageSyncTileEntity: " + ctx.side);
                return null;
            }
            ...
            {
                Autoverse.logger.error("Minecraft or player was null in MessageSyncTileEntity");
                return null;
            }
